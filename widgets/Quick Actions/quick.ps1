...
try {
    ...
    $currentY += New-SectionHeader -Text "ðŸ”§ System Maintenance" -Y $currentY
    $currentY += New-ActionButton -Text "Restart Explorer" -Icon $Icon_Restart -Y $currentY -Action {
        ...
        Update-Status "Explorer restarted âœ“" "success"
    }
    $currentY += New-ActionButton -Text "Refresh Desktop Icons" -Icon $Icon_Monitor -Y $currentY -Action {
        ...
        Update-Status "Desktop refreshed âœ“" "success"
    }
    $currentY += New-ActionButton -Text "End Non-Responding Tasks" -Icon $Icon_Warning -Y $currentY -Action {
        ...
        if ($hung) {
            ...
            Update-Status "Ended $count non-responding task(s) âœ“" "success"
        }
        else {
            Update-Status "No hung processes found âœ“" "success"
        }
    }
    $currentY += $sectionSpacing
    $currentY += New-SectionHeader -Text "ðŸ§¹ Cleanup Actions" -Y $currentY
    $currentY += New-ActionButton -Text "Clear Clipboard" -Icon $Icon_Clipboard -Y $currentY -Action {
        ...
        Update-Status "Clipboard cleared âœ“" "success"
    }
    $currentY += New-ActionButton -Text "Empty Recycle Bin" -Icon $Icon_Trash -Y $currentY -Action {
        ...
        if ($itemCount -gt 0) {
            ...
            Update-Status "Emptied $itemCount item(s) âœ“" "success"
        }
        else {
            Update-Status "Recycle Bin already empty âœ“" "success"
        }
        [System.Runtime.InteropServices.Marshal]::ReleaseComObject($shell) | Out-Null
    }
    $currentY += New-ActionButton -Text "Clear Temp Files" -Icon $Icon_Folder -Y $currentY -Action {
        ...
        Update-Status "Cleared ${sizeMB}MB temp files âœ“" "success"
    }
    $currentY += New-ActionButton -Text "Flush DNS Cache" -Icon $Icon_Globe -Y $currentY -Action {
        ...
        Update-Status "DNS cache flushed âœ“" "success"
    }
    $currentY += $sectionSpacing
    $currentY += New-SectionHeader -Text "âš¡ Quick Cleanup" -Y $currentY
    $currentY += New-ActionButton -Text "Clear Thumbnail Cache" -Icon $Icon_Image -Y $currentY -Action {
        ...
        Update-Status "Thumbnail cache cleared âœ“" "success"
    }
    $currentY += New-ActionButton -Text "Clear Windows Update Cache" -Icon $Icon_Update -Y $currentY -Action {
        ...
        Update-Status "Update cache cleared âœ“" "success"
    }
    $currentY += New-ActionButton -Text "Clear Browser Cache" -Icon $Icon_Earth -Y $currentY -Action {
        ...
        Update-Status "Browser caches cleared âœ“" "success"
    }
    $currentY += $sectionSpacing
    $currentY += New-SectionHeader -Text "âš ï¸ Clean All (Use Carefully)" -Y $currentY
    ...
    $btnCleanAll.Add_Click({
            ...
            if ($result -eq "Yes") {
                ...
                Update-Status "All cleanup tasks completed âœ“" "success"
                $this.Enabled = $true
            }
        })
