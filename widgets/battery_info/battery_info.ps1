...
$theme = @{
    ...
    Accent     = [System.Drawing.Color]::FromArgb(0, 200, 100) 
...
$iconLabel.Text = [char]::ConvertFromUtf32(0x1F50B) 
...
function Update-BatteryInfo {
    try {
        ...
        if ($null -eq $batt) {
            ...
            $iconLabel.Text = [char]::ConvertFromUtf32(0x1F50C) 
            return
        }
        ...
        if ($pct -le 20) { 
            ...
            $iconLabel.Text = [char]::ConvertFromUtf32(0x26A0) 
        }
        else { 
            $pctLabel.ForeColor = $theme.Foreground
            $iconLabel.Text = [char]::ConvertFromUtf32(0x1F50B) 
