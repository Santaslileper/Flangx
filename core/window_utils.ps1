# core/window_utils.ps1
# Windows API utilities for desktop integration
# This module is loaded ONCE and shared by all widgets

$dllPath = Join-Path (Split-Path -Parent $MyInvocation.MyCommand.Path) "DesktopWidgets.dll"

# Define the C# code first so we can check if we need to recompile
$csharp = @"
using System;
...
# If the DLL doesn't exist, OR if we want to force recompile (during dev), we add type from definition
# However, since the DLL is locked by the running process, we should just use Add-Type -TypeDefinition for the session
# To properly handle DLL updates, we would need versioning, but for this context:

if (-not ([System.Management.Automation.PSTypeName]'DesktopWidgets.WindowUtils').Type) {
    # Try using the pre-compiled DLL if it matches, but for safety in this dev cycle 
    # we will rely on Add-Type. If the Type is already loaded in the session, this block skips.
    # Note: If you change C# code, you MUST restart the PowerShell process.
...
function global:Get-OtherWidgetRects {
    ...
    $rects = @()
    # Find all powershell processes that look like widgets
